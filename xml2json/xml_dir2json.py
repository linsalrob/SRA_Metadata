...
def read_schemas(verbose=True):
    ...
    for s in schema_types:
        ...
        schemas[s] = xmlschema.XMLSchema(os.path.join("Schemas", schema_types[s]))
...
if __name__ == "__main__":
    ...
    if not os.path.exists("Schemas"):
        ...
        sys.exit(-1)
    schemas = read_schemas(True)

    if not os.path.exists(args.o):
        os.mkdir(args.o)
    
    # read all the files in the base directory
    for submission in os.listdir(args.d):
